 ...
 */
class SiteJokeSearch extends JokeWithCategory
{
    ...
    {
        $query = JokeWithCategory::find()->where(['joke_status_id' => 4]);
    ...
    public function getJokeIdsFromCategoryIds($params){
            if(!empty($params['SiteJokeSearch']['category_ids'])){

           $category_ids=[];
       
           foreach($params['SiteJokeSearch']['category_ids'] as $category_id){
